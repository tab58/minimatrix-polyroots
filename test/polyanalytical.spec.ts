...
const { Analytical: Roots } = PolynomialRootFinders;
const DEFAULT_PRECISION = 1e-14;

function expectToBeApproximatelyEqual(A: number, B: number, tol = DEFAULT_PRECISION): Chai.Assertion {
  const assertMsg = `${A} is not within tolerance to ${B}.
                      Tolerance (${tol}): ${Math.abs(A - B)}`;
  return expect(Math.abs(A - B) < tol, assertMsg).to.be.true;
}
...
describe('Cubics with Small Integer Coefficients', () => {
  it('Test 1', () => {
    const roots = Roots.getCubicRoots(1, -6, 11, -6);
  ...
  it('Test 2', () => {
    const roots = Roots.getCubicRoots(1, 0, 0, 1);
  ...
  it('Test 4', () => {
    const roots = Roots.getCubicRoots(0, 1, 3, 2);
    // 2 complex roots
    expectToBeApproximatelyEqual(roots[0].real, -1);
  ...
  it('Test 5', () => {
    const roots = Roots.getCubicRoots(1, -3, 2, 0);
  ...
  it('Test 6', () => {
    const roots = Roots.getCubicRoots(1, 3, 3, 1);
    expectToBeApproximatelyEqual(roots[0].real, -1);
    expectToBeApproximatelyEqual(roots[1].real, -1);
    expectToBeApproximatelyEqual(roots[1].imag, 0);
  ...
  it('Test 7', () => {
    const roots = Roots.getCubicRoots(1, -30, 299, -1980);
...
describe('Quartic Equations', () => {
  it('Quartic Test 1, 4 reals', () => {
    const roots = Roots.getQuarticRoots(1, -4, -1, 10, 0).sort((a: Complex, b: Complex): number => a.real - b.real);
  ...
  it('Quartic Test 2, 4 reals', () => {
    const roots = Roots.getQuarticRoots(3, 6, -123, -126, 1080).sort(
      (a: Complex, b: Complex): number => a.real - b.real,
    );
    expect(roots.length).to.be.eql(4);
  ...
  it('Quartic Test 2, 2 reals and 2 complex', () => {
    const roots = Roots.getQuarticRoots(-20, 5, 17, -29, 87).sort((a: Complex, b: Complex): number => a.real - b.real);
    expect(roots.length).to.be.eql(4);
